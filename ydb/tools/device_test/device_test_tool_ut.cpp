...
#include <kikimr/tools/device_test/proto/device_perf_test.pb.h>
...
void ProbeTest(const TString &testDescription, bool expectResults, TMaybe<NKikimr::TResultPrinter::EOutputFormat> format = {}) {
    ...
    NKikimr::TPerfTestConfig config(file.Name(), "name", "ROT", "json", 0, true);
...
Y_UNIT_TEST(PDiskTestRead) {
    ProbeTest<NDevicePerfTest::TPDiskTest, NKikimr::TPDiskTest>(GenReadCfg(), true);
}
...
Y_UNIT_TEST(PDiskTestReadAllPrinters) {
    for (auto f : { NKikimr::TResultPrinter::OUTPUT_FORMAT_WIKI,
        ...
        ProbeTest<NDevicePerfTest::TPDiskTest, NKikimr::TPDiskTest>(GenReadCfg(), false, f);
    }
}
...
Y_UNIT_TEST(PDiskTestWrite) {
    ...
    ProbeTest<NDevicePerfTest::TPDiskTest, NKikimr::TPDiskTest>(perfCfg.Str(), true);
}

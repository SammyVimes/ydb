...
#include "ydb/core/load_test/pdisk_test_actor.h"
            ...
            if (CurrentTest < TestProto.PDiskTestListSize()) {
                ...
                case NKikimr::TEvLoadTestRequest::CommandCase::kPDiskWriteLoad: {
                    ...
                    ctx.Register(CreatePDiskWriterLoadTest(cfg, ctx.SelfID, Counters,
                                CurrentTest, cfg.HasTag() ? cfg.GetTag() : 0, DiskSizeBytes));
                    break;
                }
...
public:
    TPerfTestActor(const TActorId yard, const TVDiskID vDiskID, const TPerfTestConfig& perfCfg,
            ...
            const TIntrusivePtr<NMonitoring::TDynamicCounters>& counters, ui64 diskSizeBytes)
...
struct TPDiskTest : public TPerfTest {
    ...
    void Init() override {
        ...
        try {
            ...
            IoContext->DetectFileParameters(Cfg.Path, DiskSizeBytes, IsBlockDevice);
            ...
            TActorSetupCmd testSetup(new TPerfTestActor(yardId, TVDiskID(0, 1, 0, 0, 0), Cfg, TestProto, Printer, Counters, DiskSizeBytes),
